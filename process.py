    ...
    try:
        ...
        if voice_reference and not dry_run:
            ...
            with open(voice_ref_path, "rb") as f:
                voice_reference_file = client.files.upload(
                    file=f, config={"mime_type": "audio/webm"}
                )
        ...
        for idx, clip in enumerate(clips, 1):
            ...
            try:
                ...
                context = {"travelers": travelers if travelers else []}
                ...
                if dry_run:
                    ...
                    print(
                        f"  [DRY RUN] Context: {len(travelers)} travelers, location: {context.get('location', 'N/A')}"
                    )
                    if voice_reference:
                ...
                if "error" in result:
                    ...
                    # Print brief result
                    print(
                        f"  ✓ {audio_type}, {utterance_count} utterances, {event_count} audio events"
                    )
        ...
        if dry_run:
            ...
            if audio_type_counts:
                ...
                type_summary = ", ".join(
                    [f"{count} {atype}" for atype, count in sorted(audio_type_counts.items())]
                )
